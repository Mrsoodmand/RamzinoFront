...
import PaymentFlow from "components/view/Gateway/PaymentFlow";
import LiquidityBackdrop from "components/view/Gateway/Services/LiquidityBackdrop";
...
const STEP_COUNT = 3;
// Long enough that the marker's 1.1s glide finishes and settles before the next
// step takes over; the old 2s cadence started the next hop mid-flight.
const STEP_INTERVAL = 2800;
// Decelerating ease. Motion starts fast and eases out over a long tail, which
// reads far smoother across the ~450px hops than a symmetric ease-in-out.
...
function Services() {
  ...
  // Vertical centre of the circles within the row. The row is `items-center`, so
  // where the circles land depends on how tall each column's text runs — a
  // hardcoded offset put the rail 37px above them.
  const [railCenter, setRailCenter] = useState(67);
  ...
  const goToTab = (value) => {
    ...
    window.scrollTo({
      top: wrapperTop - PIN_OFFSET + into,
      behavior: "smooth",
    });
  };
  ...
  useEffect(() => {
    ...
    // Layout offset, walked up the offsetParent chain. Unlike getBoundingClientRect
    // this ignores transforms, so it reads the circle's resting position even
    // while the entrance transition still has the column translated down.
    ...
    const measure = () => {
      ...
      if (first) {
        setRailCenter(offsetTopWithin(first, row) + first.offsetHeight / 2);
      }
    };
  ...
  // The connector rail runs from the first circle's centre to the last one's,
  // rather than edge to edge of the full-bleed row: the dashed track used to
  // spill ~180px past the outer steps and out to the viewport edges.
  const railStart = stepOffsets[0];
  const railLength = Math.max(stepOffsets[STEP_COUNT - 1] - railStart, 0);
  const railProgress = Math.max(stepOffsets[activeStep] - railStart, 0);

  return (
    <div className="">
      <section className="container mx-auto py-16 fade-in">
        <h3 className="opacity-0 animate-fade-in-up px-4 mt-12 text-center w-full text-[#0C0C0C] dark:text-[#fff] text-2xl md:text-[34px] font-black mb-5">
        ...
        <div className="grid grid-cols-12 gap-8">
          <div className="col-span-full lg:col-span-8">
            ...
            {/* Keeps its own wide, horizontal footprint — only the surface and
                type were brought in line with the three sibling cards: the
                shared dark surface (it was alone on #0B252D), the lines
                backdrop they all carry, and their title and body styles. */}
            <div className="mt-6 w-full grow">
              <div
                className="opacity-0 animate-fade-in-up flex flex-col-reverse bg-[#fff] dark:bg-[#02151B] overflow-hidden border-[0.98px] border-[#DFDFDF] dark:border-0 z-10 shadow-[0px_0px_103.41px_0px_#0000000D] lg:flex-row items-center justify-center flex-wrap lg:flex-nowrap gap-6 lg:gap-20 rounded-lg relative py-6 px-8"
                style={{ animationDelay: "520ms" }}
              >
                {/* A landscape copy of the siblings' lines backdrop, generated
                    by rotating the source SVG -90deg into a 687x400 canvas so it
                    fills this wide card instead of a portrait one. It stays
                    vector, so covering the frame costs no sharpness at any size
                    — the 399x312 raster it replaces had to be upscaled about 2x
                    here and turned blurry. The source's 1024x1024 grain overlay
                    was dropped in the process, taking the file from 711KB to
                    17KB; the noise it added is not visible at this scale. */}
                <img
                  src="/images/landing/lines-bg-wide.svg"
                  alt=""
                  className="w-full h-full absolute top-0 right-0 object-cover -z-10"
                />

                {/* The white speckle from the original artwork, restored over
                    the whole frame. It ships as a 256px tile repeated rather
                    than the source's 1024x1024 texture: random noise tiles with
                    no visible seam, and cropping instead of downscaling keeps
                ...
                />
                <div className="">
                  <span className="text-2xl md:text-2xl block text-center lg:text-right text-[#1C2121] font-bold dark:text-[#fff]">
                    اتصال سریع به وب‌سایت
                  </span>
                  <span className="mt-4 block text-center lg:text-right text-[#757878] dark:text-[#CFCFCF]">
                  ...
                  <Link
                    ...
                    className="btn btn-primary mt-8 lg:mt-4 mx-auto lg:mx-0 text-center"
                  >
                    مشاهده مستندات فنی
                  </Link>
                </div>

                <img src="/images/landing/cube.webp" alt="debit-cards" />
              </div>
            </div>
          </div>
      ...
      {/* The light pools on the right, where the copy sits (the row is RTL), and
          deepens toward the illustration on the left — so the headline keeps its
          contrast in both modes instead of sitting on the brightest part of a
          diagonal ramp. */}
      <section className="mt-20 lg:mt-40 p-1 relative fade-in bg-[radial-gradient(90%_120%_at_85%_50%,#EAFFFD_0%,#B5F7F2_38%,#6FE6E2_68%,#2FC6C8_100%)] dark:bg-[radial-gradient(90%_120%_at_85%_50%,#0F626C_0%,#0A4753_45%,#063038_75%,#041F26_100%)]">
        <LiquidityBackdrop />
        ...
        <section className="container w-full sticky z-0 fade-in">
          <div className="w-full flex items-center justify-between flex-col-reverse lg:flex-row overflow-visible rounded-[4px] sm:rounded-[12px] relative min-h-[400px]">
            <div className="z-10 px-4 md:px-[50px] pb-10 lg:pb-0">
              <h2 className="opacity-0 animate-fade-in-up text-2xl text-center text-[#000] dark:text-[#fff] lg:text-right sm:text-[38px] font-semibold leading-10 sm:leading-[50px]">
              ...
              <Link
                href="https://panel.ramzino.me/user/login"
                className="btn btn-primary btn-glass opacity-0 animate-fade-in-up mx-auto lg:mx-0 text-center"
                style={{ animationDelay: "300ms" }}
              >
                شروع فعالیت
              </Link>
            ...
            {/* In normal flow on mobile, where the row is column-reverse and so
                paints this above the copy. It used to be absolutely positioned
                at every size, which cost twice: `left-1/2` with no width made
                shrink-to-fit measure only the space from the midpoint to the
                right edge, squeezing a 574px illustration down to 173px, and
                the copy needed a hardcoded mt-[350px] to clear it — 273px more
                than the artwork actually occupied. The negative top margin
                keeps it straddling the section's top edge as before. */}
            <div className="z-20 relative -mt-20 mb-6 mx-auto w-[280px] max-w-full lg:absolute lg:mt-0 lg:mb-0 lg:w-auto lg:max-w-none lg:-top-28 lg:left-0">
              <img
      ...
      <section className="mt-20 flex flex-col-reverse lg:flex-row items-center justify-center flex-wrap lg:flex-nowrap gap-6 lg:gap-20 relative p-1 overflow-hidden py-20 fade-in">
        ...
        <div className="lg:mt-0">
          <h2 className="opacity-0 animate-fade-in-up text-2xl text-center lg:text-right sm:text-[34px] font-semibold mb-2.5 sm:mb-4 text-title leading-10 sm:leading-[50px]">
          ...
          <Link
            href="https://panel.ramzino.me/user/login"
            className="btn btn-primary opacity-0 animate-fade-in-up mt-8 mx-auto lg:mx-0 text-center"
            style={{ animationDelay: "300ms" }}
          >
            شروع فروش جهانی{" "}
          </Link>
            ...
            <div className="bg-[#fff] dark:bg-[#08242D] border border-[#EAEAEA] dark:border-[#254955] rounded-lg grid grid-cols-12 gap-4">
              {activeTab === "1" ? (
                <div className="col-span-full lg:col-span-5 p-6 flex flex-col justify-center h-full">
                  <h2 className="opacity-0 animate-fade-in-up text-2xl text-center lg:text-right sm:text-[32px] font-semibold text-title leading-10 sm:leading-[50px]">
                  ...
                  <Link
                    href="https://panel.ramzino.me/user/login"
                    className="btn btn-primary opacity-0 animate-fade-in-up mt-6 mx-auto lg:mx-0 text-center"
                    style={{ animationDelay: "300ms" }}
                  >
                    ساخت درگاه فروشگاهی
                  </Link>
              ...
              {activeTab === "2" ? (
                <div className="col-span-full lg:col-span-5 p-6 flex flex-col justify-center h-full">
                  ...
                  >
                    ...
                    پروتکل‌های رمزنگاری پیشرفته انجام می‌شوند. دارایی‌های شما در
                    کیف پول‌های سرد (Cold Storage) نگهداری می‌شوند تا از هرگونه
                    نفوذ سایبری در امان باشند.
                  </span>
                  <div className="mt-6 flex items-center gap-4 mx-auto lg:mx-0">
                    <svg
                      width="39"
              ...
              {activeTab === "3" ? (
                <div className="col-span-full lg:col-span-5 p-6 flex flex-col justify-center h-full">
                  ...
                  <span
                    ...
                    style={{ animationDelay: "150ms" }}
                  >
                    مدل درآمدی ما شفاف است. خبری از هزینه‌های پنهان، آبونمان
                    سالانه یا هزینه راه‌اندازی نیست. ما فقط درصد ناچیزی از
                    تراکنش‌های موفق&rdquo; را به عنوان کارمزد کسر می‌کنیم تا سود
                    شما حداکثر شود.
                  </span>
                  <div className="mt-6 flex items-center gap-4 mx-auto lg:mx-0">
                    <svg
      ...
      <section className="mt-20 p-1 relative fade-in">
        ...
        <div
          ref={stepsRef}
          // Below lg the three steps stack. Side by side they are 1360px wide,
          // which on a phone left a horizontal scroller showing only step 1 with
          // the rail trailing off into empty space. Stacked, all three read
          // without scrolling, and overflow stays visible so the active circle's
          // glow isn't clipped — the desktop row still needs the scroller and the
          // vertical clip it depends on.
          className="mt-12 w-full relative z-10 flex flex-col items-center gap-6 pt-2 lg:flex-row lg:items-center lg:justify-center lg:gap-20 lg:overflow-x-auto lg:overflow-y-hidden lg:pt-10 no-scrollbar"
        >
          {/* Connector rail. Everything that rides the line lives inside this
              one box, and the marker is a child of the drawn line's leading
              edge rather than a sibling with its own `right` transition — two
              separately-animated properties drifted apart mid-flight, and the
              marker also sat 2px above the line's centre. Now a single width
              animation carries both, so they cannot come apart. */}
          {/* `transition: none` is load-bearing. globals.css carries a blanket
              `html * { transition: all 0.5s ease }`, which animates this box's
              measured top/right/width. Those are layout values written straight
              from JS, not state changes worth easing, and on narrow screens the
              width transition never settled — the rail rendered at width 0 and
              the connector vanished. The line inside declares its own
              transition, so the drawn-in motion is unaffected. */}
          <div
            className="pointer-events-none absolute h-3 -z-10 hidden lg:block"
            style={{
              top: `${railCenter - 6}px`,
              right: `${railStart}px`,
              width: `${railLength}px`,
              "--rail-len": `${railLength}px`,
              transition: "none",
            }}
          >
            <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 border-t-2 border-dashed border-[#B4B4B4] dark:border-[#ffffff90]" />

            {/* Idle traffic down the track, so the rail still reads as a live
                connection during the pause between step changes. */}
            {stepsInView
              ? [0, 1, 2].map((packet) => (
                  <span
                    key={packet}
                    className="animate-rail-packet absolute right-0 top-1/2 -mt-[3px] h-1.5 w-1.5 rounded-full bg-[#6CE4DB]"
                    style={{ animationDelay: `${packet * 1200}ms` }}
                  />
                ))
              : null}

            <div
              className="absolute right-0 top-1/2 h-[3px] -translate-y-1/2 rounded-full bg-gradient-to-l from-[#16B3A7] to-[#2B758C]"
              style={{
                width: stepsInView ? `${railProgress}px` : "0px",
                opacity: stepsInView ? 1 : 0,
                transition: `width 1100ms ${GLIDE}, opacity 600ms ease-out`,
              }}
            >
              {/* Comet trail, clamped so it can't outrun a short line. */}
              <span className="absolute left-0 top-1/2 h-[3px] w-20 max-w-full -translate-y-1/2 rounded-full bg-gradient-to-r from-[#6CE4DB] to-transparent" />

              <span className="absolute left-0 top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#6CE4DB] shadow-[0_0_12px_4px_rgba(108,228,219,0.6)]">
                <span className="animate-tip-ping absolute inset-0 rounded-full bg-[#6CE4DB]" />
              </span>
            </div>
          </div>
          <div
            className={`relative flex flex-col justify-center items-center gap-6 w-full max-w-[340px] lg:w-[400px] lg:max-w-none lg:shrink transition-[opacity,transform] duration-700 ease-out ${
              stepsInView
          ...
          >
            {/* Entrance scale lives on this wrapper so its stagger delay can't
                bleed into the active-step scale below and lag every cycle. */}
            <div
            ...
            >
              <span
                className={`pointer-events-none absolute inset-0 rounded-full border border-[#6CE4DB] ${
                  stepsInView && activeStep === 0
                    ? "animate-halo-pulse"
                    : "opacity-0"
                }`}
              />
              <div
                ...
                className="relative w-[65px] h-[65px] rounded-full flex items-center justify-center bg-[radial-gradient(288.22%_288.22%_at_49.92%_50%,#16B3A7_0%,#2B758C_55%)] border border-[#EEEEEE]"
                style={{
                  transform: activeStep === 0 ? "scale(1.07)" : "scale(1)",
                  transition: `transform 600ms ${GLIDE}, box-shadow 600ms ease-out`,
                  boxShadow:
                    activeStep === 0
                      ? "0 0 0 3px rgba(108,228,219,0.18), 0 0 26px 6px rgba(108,228,219,0.30)"
                      : "0 0 0 0px rgba(108,228,219,0)",
                }}
              >
                <span
                  className={`absolute -bottom-1 -left-1 flex h-5 w-5 items-center justify-center rounded-full text-[11px] font-bold transition-colors duration-500 ease-out ${
                    activeStep === 0
                      ? "bg-[#6CE4DB] text-[#02323B]"
                      : "bg-[#E8EDEE] text-[#7E8C90] dark:bg-[#1E333A] dark:text-[#8FA3AB]"
                  }`}
                >
                  ۱
                </span>
            ...
            <span
              className={`text-[15px] text-center sm:text-2xl font-semibold transition-colors duration-500 ease-out ${
                activeStep === 0
                  ? "text-[#158C93] dark:text-[#6CE4DB]"
                  : "text-title dark:text-[#fff]"
              }`}
            >
              ثبت نام در سایت
            </span>
          ...
          {/* Stacked-only connector, standing in for the horizontal rail. */}
          <span
            aria-hidden="true"
            className="h-10 border-l-2 border-dashed border-[#B4B4B4] dark:border-[#ffffff90] lg:hidden"
          />
          <div
            className={`relative flex flex-col justify-center items-center gap-6 w-full max-w-[340px] lg:w-[400px] lg:max-w-none lg:shrink transition-[opacity,transform] duration-700 ease-out ${
              stepsInView
          ...
          >
            ...
            >
              ...
              <div
                ...
                className="relative w-[65px] h-[65px] rounded-full flex items-center justify-center bg-[#fff] border border-[#EEEEEE] dark:bg-[#1E333A] dark:border-0"
                style={{
                  transform: activeStep === 1 ? "scale(1.07)" : "scale(1)",
                  transition: `transform 600ms ${GLIDE}, box-shadow 600ms ease-out`,
                  boxShadow:
                    activeStep === 1
                      ? "0 0 0 3px rgba(108,228,219,0.18), 0 0 26px 6px rgba(108,228,219,0.30)"
          ...
          >
            ...
            >
              ...
              <div
                ...
                className="relative w-[65px] h-[65px] rounded-full flex items-center justify-center bg-[#fff] border border-[#EEEEEE] dark:bg-[#1E333A] dark:border-0"
                style={{
                  transform: activeStep === 2 ? "scale(1.07)" : "scale(1)",
                  transition: `transform 600ms ${GLIDE}, box-shadow 600ms ease-out`,
                  boxShadow:
                    activeStep === 2
                      ? "0 0 0 3px rgba(108,228,219,0.18), 0 0 26px 6px rgba(108,228,219,0.30)"
      ...
      <section className="mt-20 container fade-in">
        <div className="bg-[#fff] overflow-hidden dark:bg-[#0B252D] border-[0.98px] border-[#DFDFDF] dark:border-0 z-10 shadow-[0px_0px_103.41px_0px_#0000000D] rounded-lg relative py-6 px-8">
          ...
          <div className="flex flex-col-reverse lg:flex-row items-center justify-evenly flex-wrap lg:flex-nowrap gap-6 lg:gap-20">
            <div className="">
              <h2 className="opacity-0 animate-fade-in-up text-2xl text-center lg:text-right sm:text-[34px] font-semibold text-title leading-10 sm:leading-[50px]">
              ...
              <Link
                href="https://ramzino.me/docs"
                className="btn btn-primary opacity-0 animate-fade-in-up mx-auto lg:mx-0 text-center"
                style={{ animationDelay: "300ms" }}
              >
                مستندات فنی{" "}
              </Link>
            ...
            <img src="/images/landing/api-vector.webp" alt="api-vector" />
          </div>
      ...
      <section className="mt-20 w-full full-center flex-col pt-5 relative overflow-hidden fade-in">
        <h3 className="opacity-0 animate-fade-in-up px-4 text-center w-full text-[#0C0C0C] dark:text-[#fff] text-2xl md:text-[34px] font-black mb-5">
      ...
      <section className="mt-32 p-1 relative flex items-center justify-center h-[800px] bg-gradient-to-tl from-[#FAFAFA] via-[#E6F9F9] to-[#9EFBFB] dark:from-[#02151B] dark:via-[#0E3F47] dark:to-[#146B6B] fade-in">
        ...
        <div className="flex flex-col justify-center items-center gap-6 w-[400px] shrink-0 lg:shrink mx-auto">
          ...
          <Link
            href={"#"}
            className="btn btn-primary opacity-0 animate-fade-in-up w-full py-4"
